...
import {Player, Role} from '../../stores/slices';

const ShowCards = () => {
  const language = useAppSelector(state => state.App.language, shallowEqual);
  const nav = useNavigation();
  const rolePlayers = useAppSelector(
    state => state.Game.rolePlayers,
    shallowEqual,
  );
  ...
  return (
    <View
          ...
          return (
            <View style={styles.emptyList}>
              ...
              <Text style={{fontSize: 20, marginTop: 40}}>
                {t('game.allPlayersSeenTheirRoles')}
              </Text>
            </View>
          );
        }}
      ...
      <Pressable
        ...
        <Text style={{fontSize: 18}}>{t('game.goToTheGame')}</Text>
      </Pressable>
      <ReactNativeModal
        isVisible={showModal}
        onBackButtonPress={() => {
        ...
        <View
          ...
          <View
            ...
            <Text style={{fontSize: spacing.lg}}>
              {t(detailPlayer?.role?.title as string)}
            </Text>
