...
import {NightStory, Player, Role} from '../../stores/slices';

const GameNightPlay = () => {
  const nav = useNavigation();
  const {night, nightStory, rolePlayers} = useAppSelector(
    state => state.Game,
    shallowEqual,
  );
  ...
  >();
  const [nightWord, setNightWord] = useState<string>('');
  const [toNight, setToNight] = useState<NightStory[]>([]);
      ...
      <BottomSheet
        ...
        <BottomSheetView style={[styles.contentContainer]}>
          ...
          <Text style={{fontSize: 18, marginBottom: 10}}>
            {toNight[0]?.jobs[0]?.role} بیدار شود
          </Text>
          <Text style={{fontSize: 15, marginBottom: 10}}>
            {toNight[0]?.description}
          </Text>
        </BottomSheetView>
      </BottomSheet>
    </View>
