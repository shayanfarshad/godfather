...
import {DHeight, DWidth, getDayWord} from '../../constants/Constants';
...
const GamePlay = () => {
  ...
  useEffect(() => {
    if (gameType === 'jack') {
      setLastMoves(jackLastMove);
    } else {
      setLastMoves(nustraLastMove);
    }
  }, [gameType]);
  ...
  const removeItem = () => {
    ...
    if (clickedCard) {
      const newCards = lastMoves.filter((el: any) => el.id !== clickedCard.id);
  ...
  const [routes] = React.useState([
    {key: 'inside', title: t('game.insideGame')},
    {key: 'outside', title: t('game.sitoutGame')},
  ]);
  ...
  const resetGameAndExit = useCallback(() => {
    dispatch(gameActions.gameReset());
    navigate('RPM', {screen: 'myPlayers'});
  }, []);

  return (
    <View style={{flex: 1, paddingTop: 10, backgroundColor: colors.background}}>
      <View style={styles.header}>
        <Text style={{fontSize: 20}}>{t('game.day')}</Text>
        <View
          style={{
          ...
          <Pressable
            ...
            <Text style={{fontSize: 20}}>{t('game.lastMove')}</Text>
          </Pressable>
          <Pressable
            onPress={() => {
      ...
      <Modal
        ...
        <View style={styles.selectImageContainer}>
          <View>
            <Text style={{fontSize: 20}}>{t('game.doYouWantToCloseGame')}</Text>
          </View>
          <View
            style={{
              flexDirection: 'row',
...
const CardList = ({item, setClickedCard, index}: any) => {
